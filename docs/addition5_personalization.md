...
Paule-Mandel τ² random-effects pooling, the post-T2-6 default of the
`within_person_cstatistic` helper at `experiment/5/_personal/within_person.py`,
regenerated by `python experiment/5/run_personalization.py --cv` with the
output summary at `experiment/5/within_person_summary_cv_20260530_151055.csv`.
This matches the body §3.6 and abstract Results primary disclosure. The
pre-T2-6 DerSimonian-Laird τ² sensitivity is available alongside at the body
§3.6 L93 canonical side-by-side disclosure (Paule-Mandel primary + DL
sensitivity quoted explicitly for the two headline TabPFN cells); the two
estimators agree at the third decimal place on this cohort with the largest
DL-vs-PM shifts being the migraine XGBoost point estimate (DL 0.554 → PM
0.558) and the headache sequence CI lower bound (DL 0.505 → PM 0.506).
