"""
5-fold time-series cross-validation for sequence (version_gru).

Expanding-window TimeSeriesSplit on diary_cv5_timeseries.parquet (target-level,
ratio-independent). This is Addition 4's PRIMARY internal-validation estimate:
on ~201 positive migraine days a single 15% hold-out window is too small and
too time-period-dependent for a stable point estimate, so the head-to-head
comparison against the Additions 0/1 tabular cells leads on the across-fold CV
numbers; the single chronological hold-out (evaluate.py) is the locked check.
See docs/addition4_sequence.md Section 3.4.

Per fold (leakage-safe, temporally ordered):
  train_sub (first 80% of training-fold dates)  -> fit the sequence model
  cal_sub   (last  20% of training-fold dates)  -> select operating thresholds
  evaluation (cv_fold == k)                      -> score only

Windowing happens inside the estimator and within each frame, so a window in
cal_sub / the evaluation fold never draws look-back days from an earlier fold
(the row-level analogue of the leakage discipline in _seq/windowing.py).
...
# Shared imports. _seq on the path so the architecture's nn.Module + the
# SequenceClassifier (refit per fold here) resolve.
...
from _model_architecture.gru.model import build_gru  # noqa: E402
...
TITLE         = (
    "STAGE 4 / park_features / sequence (version_gru) - "
...
def _main_inner():
    ...
    cv = load_raw(CV_PATH, loader=select_park_features)
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        if train_fold.empty:
            print("  (no training data before this fold; skipping)")
            continue

        train_sub, cal_sub = chronological_subsplit(train_fold, cal_ratio=CAL_RATIO)

        # seq_split keeps patient_id + date so the estimator can window.
        X_train_sub, y_train_sub = seq_split(train_sub)
        X_cal_sub,   y_cal_sub   = seq_split(cal_sub)
        X_val,       y_val       = seq_split(val_fold)
        ...
        print(f"  Fitting sequence model on train_sub (windows built inside the estimator)...")
        model = build_gru(X_train_sub, y_train_sub, output_dir=EXPERIMENT_DIR)
