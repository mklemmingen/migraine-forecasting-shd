...
def main():
    with capture_training_output(EXPERIMENT_DIR, label='training'):
        ...
        X_train, y_train = load_seq(TRAIN_PATH, loader=select_park_features)

        print(f"Train set: X={X_train.shape}, y={y_train.shape}")

        print("Fitting sequence (version_gru) - windows built inside the estimator...")
        model = build_gru(X_train, y_train, output_dir=EXPERIMENT_DIR)
