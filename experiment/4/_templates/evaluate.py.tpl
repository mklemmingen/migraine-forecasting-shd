...
# Shared imports. _seq is on the path so the pickled SequenceClassifier
# (module `sklearn_wrapper`, referencing `windowing`) and the architecture's
# nn.Module (in _model_architecture/<arch>/model.py) reload cleanly.
...
{extra_imports}from _seq.dataread import load_seq  # noqa: E402
...
TITLE         = "STAGE {addition} / {feature_set} / sequence (version_{version_label})"


def main():
    print("Loading datasets and model (id-bearing X for windowing)...")
    X_val,  y_val  = load_seq(VAL_PATH,  loader={raw_loader})
    X_test, y_test = load_seq(TEST_PATH, loader={raw_loader})
    model = joblib.load(MODEL_PATH)

    print("Generating predictions (windows built inside the estimator)...")
